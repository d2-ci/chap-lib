            ...
            evaluation: Array.from(new Set(data.map((item) => item.orgUnit))).map((orgUnit) => {
                return {
                    ...
                    models: modelNames.map((mn) => {
                        ...
                        data.filter((o) => o.orgUnit === orgUnit &&
                            o.splitPeriod === splitPeriod &&
                            (o?.modelName || modelName) === mn), quantileFunc);
...
export function createHighChartsData(plotData, quantileFunc) {
    ...
    periods.forEach((period) => {
        const quantileLow = plotData.find((item) => item.period === period &&
            quantileFunc(item) === 'quantile_low')?.value || 0;
        const quantileHigh = plotData.find((item) => item.period === period &&
            quantileFunc(item) === 'quantile_high')?.value || 0;
        const median = plotData.find((item) => item.period === period && quantileFunc(item) === 'median')?.value || 0;
        const quantileMidHigh = plotData.find((item) => item.period === period &&
            quantileFunc(item) === 'quantile_mid_high')?.value || 0;
        const quantileMidLow = plotData.find((item) => item.period === period &&
            quantileFunc(item) === 'quantile_mid_low')?.value || 0;
